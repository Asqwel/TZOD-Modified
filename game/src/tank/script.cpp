...
int luaT_pushobj(lua_State *L)
{
	...
	float pow = (float) luaL_checknumber(L, 3);
	angle = abs(-angle);
	who->ApplyForce(vec2d(angle) * pow);
	return 0;
}

int luaT_rotateobj(lua_State *L)
{
	int n = lua_gettop(L);
	if ( 2 != n )
	{
	...
	GC_RigidBodyDynamic *who = luaT_checkobjectT<GC_RigidBodyDynamic>(L, 1);
	float momentum = (float) luaL_checknumber(L, 2);
	who->ApplyMomentum(momentum);
	return 0;
}
