...
void GC_RigidBodyStatic::Invulnerable(int check)
{
	assert(_invulnerable >= 0);
	_invulnerable = check;
}
...
bool GC_RigidBodyStatic::TakeDamage(float damage, const vec2d &hit, GC_Player *from)
{
	...
	if( _health_max > 0 && !_invulnerable )
	{
...
void GC_RigidBodyStatic::MapExchange(MapFile &f)
{
	...
	MAP_EXCHANGE_INT(invulnerable, _invulnerable, 0);
...
GC_RigidBodyStatic::MyPropertySet::MyPropertySet(GC_Object *object)
  ...
  , _propInvulnerable( ObjectProperty::TYPE_INTEGER,    "invulnerable"  )
{
	_propMaxHealth.SetFloatRange(0, 100000);
	_propHealth.SetFloatRange(0, 100000);
	_propInvulnerable.SetIntRange(0, 1);
}
...
ObjectProperty* GC_RigidBodyStatic::MyPropertySet::GetProperty(int index)
{
	...
	case 4: return &_propInvulnerable;
	}
...
void GC_RigidBodyStatic::MyPropertySet::MyExchange(bool applyToObject)
{
	...
	if( applyToObject )
	{
		...
		tmp->Invulnerable( _propInvulnerable.GetIntValue() );
	}
	else
	{
		...
		_propInvulnerable.SetIntValue(tmp->IsInvulnerable());
	}
}
